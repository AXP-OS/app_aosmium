...
if [ $gsync -eq 1 ]; then
    ...
    sed -E -i 's/^(\s+default_min_sdk_version\s+=\s+)[0-9]+$/\128/g' build/config/android/config.gni # set default minimum sdk to A9
    sed -i 's/_min_sdk_version = invoker.min_sdk_version/_min_sdk_version = default_min_sdk_version/g' build/config/android/rules.gni # enforce minimum sdk
